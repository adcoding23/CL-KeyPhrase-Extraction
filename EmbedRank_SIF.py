...
# -*- coding: utf-8 -*-
"""
Created on Tue Dec 10 13:42:35 2019
...
def get_sentence_embeddings(sentences, phrase = True, ngram='bigrams', model='concat_wiki_twitter', a = 1e-3):
    ...
    if model == "wiki" or model == 'concat_wiki_twitter':
        ...
        #Getting frequency count for SIF
        word_counts = Counter(itertools.chain(*sentences))
  
    
        #GETTING EMBEDDINGS USING FASTTEXT
        if ngram == 'unigrams':
    ...
    #SIF   
    embedding_size = len(wiki_embeddings)
    vs = np.zeros(embedding_size)
    #sentence_length = len(sentences)
    a_value = 0
    count = 0
    for word in tokenized_sentences_SNLP:
        a_value = a / (a + word_counts[word]) # smooth inverse frequency, SIF
        try:
            vs = np.add(vs, np.multiply(a_value, wiki_embeddings[word])) # vs += sif * word_vector
            count +=1
        except Exception as e:
            print("error", e)
            #print(e)
        pass
    vs = np.divide(vs, count) # weighted average
    print("VS: ", vs)
...
if __name__ == '__main__':
        ...
        mypath = '/home/shreya/Documents/CMSC723/yutaya/EmbedRank/embedrank/Hulth2003/train_sif'
        write_path = '/home/shreya/Documents/CMSC723/yutaya/EmbedRank/embedrank/output_sif/'
